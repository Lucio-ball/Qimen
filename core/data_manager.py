...
class DataManager:
    ...
    def __init__(self, db_path: str = "qimen_cases.db"):
        """
        初始化数据管理器
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if case.id is None:
                    # 新案例 - 插入
                    cursor.execute('''
                        INSERT INTO cases (name, creation_time, query_time, chart_result_json, annotation_layers_json)
                        VALUES (?, ?, ?, ?, ?)
                    ''', (case.title, current_time, query_time, chart_result_json, annotation_layers_json))
                    ...
                    cursor.execute('''
                        UPDATE cases 
                        SET name = ?, query_time = ?, chart_result_json = ?, annotation_layers_json = ?
                        WHERE id = ?
                    ''', (case.title, query_time, chart_result_json, annotation_layers_json, case.id))
    ...
    def load_case(self, case_id: int) -> Optional[Case]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    SELECT id, name, creation_time, query_time, chart_result_json, annotation_layers_json
                    FROM cases WHERE id = ?
                ''', (case_id,))
                ...
                # 解析数据
                case_id, name, creation_time, query_time, chart_result_json, annotation_layers_json = row
    ...
    def get_all_cases_summary(self) -> List[Dict]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('''
                    SELECT id, name, query_time, creation_time
                    FROM cases
